...
/**
 * NForm control for selecting date.
 *
 ...
 */
class DatePicker extends NFormControl
{
	...
	public function setValue($value)
	{
		...
		if ($value !== NULL) {
			...
			try {
				$value = NTools::createDateTime($value); // clone DateTime when given
			} catch (Exception $e) {
				$value = NULL;
			}
		}
	 ...
	 */
	private function extractRangeRule(NRules $rules)
	{
		$controlMin = $controlMax = NULL;
		foreach ($rules as $rule) {
			if ($rule->type === NRule::VALIDATOR) {
				if ($rule->operation === NForm::RANGE && !$rule->isNegative) {
					$ruleMinMax = $rule->arg;
				}

			} elseif ($rule->type === NRule::CONDITION) {
				if ($rule->operation === NForm::FILLED && !$rule->isNegative && $rule->control === $this) {
